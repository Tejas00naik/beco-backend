...
This module contains helper functions used by the payment processing modules.
"""
...
async def check_document_exists(dao, collection: str, conditions: Dict[str, Any]) -> bool:
    """
    Check if a document exists in a collection based on multiple conditions.
    
    Args:
        dao: Firestore DAO instance
        collection: Firestore collection name
        conditions: Dictionary of field-value pairs to match
        
    Returns:
        True if a matching document exists, False otherwise
    """
    try:
        # Convert conditions to list of tuples for query_documents
        query_conditions = [(k, "==", v) for k, v in conditions.items()]
        results = await dao.query_documents(collection, query_conditions)
